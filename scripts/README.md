...
| `backtest_race_forecast.py` | **The outcome scorecard.** Two rank tests on the pace+grid+passability blend, then a full probabilistic replay: Brier / log loss on `p_win`, `p_podium`, `p_points`, `p_dnf`, MAE on `e_finish`, reliability bins and a calibration slope — scored against climatology, a leak-free expanding-window grid baseline, and the betting market. Every gap carries a **cluster bootstrap on races**. Writes `data/backtest_race_forecast_detail.csv` + `data/backtest_race_forecast.csv`. Args: `--seasons`, `--rank-tests-only`, `--report`. | After changing `f1lib/race_forecast.py` or `driver_ratings.py`. Slow (~105 races) so it is **not** in the `after_race.py` chain; `--report` re-scores the saved detail in seconds. Current verdict in read_local.md — only `p_points` significantly beats the grid baseline, and the predictions are under-separated. |
| `fetch_odds.py` | Snapshots market-implied probabilities (Kalshi, no account needed) → `data/odds_snapshots.csv`, append-only. Stores the raw price **and** three de-vigged probabilities. Args: `--backfill`, `--all`, `--status`, `--dry-run`. | Per session from `during_weekend.py` (step 5); first in the `after_race.py` chain, where `--backfill` reconstructs the whole weekend hourly from candlesticks. No scheduled task by design — `..\odds-snapshot.ps1 -Install` adds one if Kalshi ever closes that endpoint. **Time-critical** — a price exists only while the market is open, and Kalshi prunes a race after ~7 weekends, so history older than ~2 months is unrecoverable. When analysing, filter on `pre_lock`, never `close_time` (that is settlement, hours after the flag). Never feed it to a model: it is the benchmark the outcome model gets scored against. |
| `fetch_odds.py --source polymarket` | Historical backfill from Polymarket → same CSV, `bookmaker=polymarket`. Reaches **2024–2026**, i.e. the races Kalshi has already pruned. **Resumable** — re-run any time and it collects only the gap. | Whenever the VPN is up (non-French exit) and `after_race.py` reports missing races; it prints that gap for you. Raises a named `PolymarketBlocked` if the VPN is off, so the failure says what to do. Not urgent — Polymarket keeps markets indefinitely — but resolution decays with age (hourly → 12-hourly), so sooner is better. On this source filter **`overround` (0.9–1.25) as well as `pre_lock`**: an untraded 12h candle returns a flat 0.50 and silently inflates the book. |
